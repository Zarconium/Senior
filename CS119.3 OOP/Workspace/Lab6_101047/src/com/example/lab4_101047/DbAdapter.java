...
public class DbAdapter
{
	...
	public void editMovie(String title, String gross, String year)
	{
		ContentValues values = new ContentValues();
		values.put(KEY_GROSS, gross);
		values.put(KEY_YEAR, year);	
		
		mDb.update(SQLITE_TABLE, values, KEY_TITLE + "='" + title + "'", null);
	}

	public Cursor fetchMoviesByTitle(String inputText) throws SQLException
	{
		Cursor mCursor = null;
		...
		{
			mCursor = mDb.query(true, SQLITE_TABLE, new String[] { KEY_ROWID, KEY_TITLE, KEY_GROSS, KEY_YEAR }, KEY_TITLE + " LIKE '%" + inputText + "%'", null, null, null, null, null);
		}
		if (mCursor != null)
		{
			mCursor.moveToFirst();
	...
	public Cursor fetchMoviesByYear(String inputText) throws SQLException
	{
		Cursor mCursor = null;
		if (inputText == null || inputText.length() == 0)
		{
		...
		{
			mCursor = mDb.query(true, SQLITE_TABLE, new String[] { KEY_ROWID, KEY_TITLE, KEY_GROSS, KEY_YEAR }, KEY_YEAR + " LIKE '%" + inputText + "%'", null, null, null, null, null);
		}
		if (mCursor != null)
		{
			mCursor.moveToFirst();
